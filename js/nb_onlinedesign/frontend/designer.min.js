...
(function() {
    ...
    canvas.on("object:selected", function(options) {
        ...
        else if (toType(object.getFill()) ===
            "object") scope.editable.pattern = true;
    ...
    angular.module("app", ["LocalStorageModule", "angularFileUpload", "imgpreload"]).constant("_", window._).config(function(localStorageServiceProvider) {
    ...
    $.confirm({
        ...
        confirm: function() {
            ...
            if (index == null) {
                ...
                scope.editableItem = null;
                scope.closePopover()
            } else scope.canvas.remove(scope.canvas.item(index));
            scope.drawCanvas();
            scope.saveDesign();
...
function deleteAllObject() {
    $.confirm({
        ...
        confirm: function() {
			...
			$("#image_dimension").css("display", "none");
			$("#image_filter1").css("display", "none");
			$("#image_filter2").css("display", "none");
			$("#image_filter3").css("display", "none");
			$("#image_general").css("display", "none");
			$("#free_draw").css("display", "none");
			$("#draw_shape").css("display", "none");
			$("#svg_path").css("display", "none");
			$("#svg_config").css("display", "none");
			
			$("#config_text").css("display", "none");
			$("#config_art").css("display", "none");
			$("#config_draw").css("display", "none");
			$("#config_image").css("display", "none");
			$(".hide-tool-config").css("display", "none");
			
			$("#typography textarea").text("");
		},
        cancel: function() {}
    })
}
...
angular.module("app").controller("DesignerController", function($scope, PatternService, ProductService, localStorageService, FontService, ArtService, QrcodeService, $interval, $upload, $timeout, LanguageService) {
    $scope.canvas = null;
    ...
    $scope.colorOptional = "cc324b";
    $scope.colorStrokeOptional = "394264";
    $scope.background = "f98332";
    ...
    $scope.file = [];
    $scope.loading = false;
    $scope.currentCateFont = "Font Categories";
    $scope.contentText = "Text Here";
    $scope.designScale = calcCanvasWidth() / 300;
    $scope.scale = {};
    $scope.rateConvertCm2Px = 37.8;
    $scope.currentScale = "1";
    $scope.iid = "images";
    $scope.qrCodeContent = "http://example.com/";
    ...
    $scope.currentCatArtName = "Categories";
    ...
    $scope.currentCatFontName = "Categories";
    $scope.countFont = 0;
    $scope.enableGroup = false;
    $scope.colorBrush = "cc324b";
    $scope.brushShadowWidth = 1;
    $scope.brushWidth = 1;
    $scope.brushOpacity = 1;
    $scope.brushAngle = 0;
    $scope.colorShape = "f98332";
    ...
    $scope.resetDimension = function() {
        ...
        $scope.ajustmentOrientationScale[index] =
            $scope.designScale;
    ...
    $scope.addText = function() {
        ...
        $scope.zoom = 1;
        $scope.zoomCanvas();
        var text = new fabric.Text($scope.contentText, {
            left: 100,
            top: 10,
            fill: "#cc324b",
            fontFamily: "Arial",
    ...
    $scope.changeShadow = function() {
        if ($scope.editableItem == null) return;
        var color = hexToRGB("#" + $scope.shadow.color);
    ...
    $scope.setBackground = function(e) {
        if ($scope.editableItem ==
            null) return;
        if (!e) $scope.canvas.item($scope.editableItem).setTextBackgroundColor("#" + $scope.background);
    ...
    $scope.loadProduct = function() {
        ProductService.getProduct().then(function(data) {
            ...
            var font_url = window.parent.nbds_frontend.font_used_url;
			$scope.loadAdminDesignFont(data.fonts, font_url);
			/*fix resize*/
			if (toType(data.config) === "object") $scope.ajustmentScale = data.config["scale"];
			/*end. fix resize*/
			if (toType(data.design) == "object") $scope.canvases = data.design;
            else {
                var sid = "p_" + product_id,
					cid = "c_" + product_id;
                if (toType(localStorageService.get(sid)) === "object") {
					/*fix resize*/
	...
	$scope.loadAdminDesignFont = function(fonts, _url) {
        ...
        var stop = $interval(function() {
            ...
            _.each(fonts, function(font, key) {
                if (!isFontAvailable(font.name)) check[key] = false;
                else check[key] = true
            });
    ...
    $scope.loadLanguage = function(code) {
        ...
        LanguageService.getLang(code).then(function(data) {
            if (data.mes ==
                ...
                langjs["DELETE_ITEM_MESSAGE"] = $scope.langs["DELETE_ITEM_MESSAGE"] ? $scope.langs["DELETE_ITEM_MESSAGE"] : "Are you sure you want to delete this item?";
                langjs["DELETE_ALL_ITEM_MESSAGE"] = $scope.langs["DELETE_ALL_ITEM_MESSAGE"] ? $scope.langs["DELETE_ALL_ITEM_MESSAGE"] : "Are you sure you want to delete all layers?";
                langjs["LAYER_IMAGE"] =
                    $scope.langs["IMAGE"] ? $scope.langs["IMAGE"] : "Image";
                langjs["LAYER_GROUP"] = $scope.langs["GROUP"] ? $scope.langs["GROUP"] : "Group";
                langjs["LAYER_PATH"] = $scope.langs["PATH"] ? $scope.langs["PATH"] : "Path";
                langjs["LAYER_RECTANGLE"] = $scope.langs["RECTANGLE"] ? $scope.langs["RECTANGLE"] : "Rectangle";
                langjs["LAYER_CIRCLE"] = $scope.langs["CIRCLE"] ? $scope.langs["CIRCLE"] : "Circle";
                langjs["LAYER_TRIANGLE"] = $scope.langs["TRIANGLE"] ? $scope.langs["TRIANGLE"] : "Triangle";
                langjs["LAYER_LINE"] = $scope.langs["LINE"] ? $scope.langs["LINE"] :
                    "Line";
                langjs["LAYER_POLYGON"] = $scope.langs["POLYGON"] ? $scope.langs["POLYGON"] : "Polygon"
            }
    ...
    $scope.changeFontPageSize = function(first) {
        ...
        if ($scope.curentCatFont == googleCatFont) {
    ...
    $scope.changeFontCat = function(cat) {
        var first = false;
        if (cat.id == googleCatFont) $scope.AllFonts = $scope.google_font;
        ...
        if (cat.id == googleCatFont) $scope.countFont = $scope.google_font.length;
        else {
            $scope.getAmountFont($scope.fonts, $scope.curentCatFont);
    ...
    $scope.loadArt = function() {
        ...
        ArtService.fn(function(data) {
            ...
            if (data.flag == "1") {
				...
				console.log(toType(data.arts));
				console.log($scope.arts);
            } else console.log("Error! Try again!")
        })
    };
    $scope.changeArtCat = function(cat) {
        $scope.artPageSizes[$scope.curentCatArt] =
            $scope.artPageSize;
    ...
    $scope.addArt = function(art) {
        fabric.loadSVGFromURL(art.url, function(ob, op) {
            ...
            var index = $scope.canvas.getObjects().length - 1;
            $scope.addLayers($scope.canvas.getActiveObject(), "path-group", index);
    ...
    $scope.getColorPath = function(paths) {
        ...
        _.each(paths, function(path, key) {
            if (path.fill.charAt(0) == "#") {
                ...
                $scope.pathColor[key] = color
            } else if (path.fill.charAt(0) == "r") {
                var rgb = path.fill.slice(4, path.fill.length - 1);
                var arr = rgb.split(",");
                var check = true;
                if (arr[0] == "0" && arr[1] == "0" && arr[2] == "0" && !!arr[3]) check = false;
                if (check) {
                    var R = parseInt(arr[0], 16),
                        G = parseInt(arr[1], 16),
                        B = parseInt(arr[2], 16);
                    var color = R + "" + G + "" + B;
                    output.push({
    ...
    $scope.updatePathColor = function(color) {
        if ($scope.editable.type == "path-group") $scope.editable._paths[color.key].fill = "#" + $scope.pathColor[color.key];
        $scope.drawCanvas()
    };
    ...
    $scope.activeLayer = function(e) {
        if ($scope.canvas.item(e.index).get("selectable")) {
    ...
    $scope.changeOrientation = function(p) {
        ...
        if (angular.isDefined($scope.canvases[p.name])) $scope.canvas.loadFromJSON($scope.canvases[p.name],
            function() {
                /* fix resize */
                applyImageFilters();
                $scope.canvas.renderAll();
                ...
                } else if ($scope.ignoreLocalDesign) $scope.resizeDesign($scope.designScale, $scope.ajustmentScale);
        ...
        else $scope.canvas.clear();
		if (angular.isDefined($scope.fontCollection[p.name])) $scope.currentFontCollection = JSON.parse($scope.fontCollection[p.name]);
        $scope.zoom = 1;
        $scope.zoomCanvas();
        $scope.drawCanvas();
    ...
    $scope.usedFont = function(cv) {
        ...
        cv.forEachObject(function(object) {
            if (object.type === "i-text" || object.type === "text" || object.type === "curvedText") font.push(object.fontFamily)
        });
        return font
    };
	...
	$scope.filterFontUsed = function() {
        ...
        _.forEach($scope.fontCollection, function(value) {
            ...
            _.each(fonts, function(v) {
                if (toType(v) == "object")
                    if (v.cat[0] == googleCatFont) usedFonts.push({
                        "type": "google",
                        "name": v.name
                    });
                    else usedFonts.push({
    ...
    $scope.storeDesign1 = function() {
        ...
        _.each($scope.canvases, function(value, key) {
            ...
            if (/MSIE/.test(navigator.userAgent)) images[key] =
                $scope.canvas.deactivateAll().toSVG();
        ...
        UserService.getUserId().then(function(data) {
            ...
            if ($scope.currentUser !== "" || $scope.currentUser > 0)
                ...
                else var query = "type=saveDesign";
    ...
    $scope.onFileSelect = function($files) {
        ...
        for (var i = 0; i < $files.length; i++) {
            if ($files[i].type.indexOf("image") === -1) {
                $scope.errorUpload = "Only images are allowed";
                continue
            }
            if (window.parent.nbds_frontend.upload_max) max_size = window.parent.nbds_frontend.upload_max;
            if ($files[i].size > max_size * 1024 * 1024) {
                $scope.errorUpload = "Oops, file upload larger than " +
                    window.parent.nbds_frontend.upload_max + " MB";
                continue
            }
        ...
        if ($scope.errorUpload == "") {
            ...
            for (var i = 0; i < $scope.file.length; i++) {
                ...
                $scope.upload = $upload.upload({
                    url: window.parent.nbds_frontend.upload_image_url,
                    method: "POST",
                    withCredentials: true,
                    data: {
                        "src": $scope.src,
                        "action": "nbdesigner_customer_uploadAction",
	...
	$scope.loadLocalStorageImage = function() {
        $scope.disableDrawMode();
        var localSource =
            localStorageService.get($scope.iid, $scope.uploadURL);
        if (!!localSource && toType(localSource) == "array") $scope.uploadURL = localSource;
        $scope.loaded = 30
    };
    ...
    $scope.initWebcam = function(){
        ...
        Webcam.setSWFLocation(window.parent.nbds_frontend.url_style + 'webcam.swf');  
    };
    ...
    $scope.addImage = function(url) {
        ...
        fabric.Image.fromURL(url, function(op) {
            ...
            var index = $scope.canvas.getObjects().length - 1;
            $scope.addLayers($scope.canvas.getActiveObject(), "image", index);
            $scope.canvas.setActiveObject($scope.canvas.item(index));
            $scope.drawCanvas()
        })
    };
    $scope.addImageFromUrl = function() {
        ...
        fabric.util.loadImage($scope.imageFromUrl, function(img) {
            if (img == null) alert("Error!");
                ...
                jQuery.ajax({
                    url: window.parent.nbds_frontend.copy_img_url,
                    method: "POST",
                    data: {
    ...
    $scope.loadSource = function() {
        var localSource = localStorageService.get($scope.iid, $scope.uploadURL);
        if (!!localSource &&
            toType(localSource) == "array") $scope.uploadURL = localSource;
        $scope.loaded = 30
    };
    ...
    $scope.debug = function() {
        console.log(_.size($scope.currentVariant.info))
    };
    $scope.state = "pub";
    window.scope = $scope
});
angular.module("app").factory("ArtService", function($http) {
    return {
        fn: function(callback) {
            ...
            $http({
                method: "POST",
                url: window.parent.nbds_frontend.load_cat_url,
                params: {
                    action: "nbdesigner_get_artAction",
...
angular.module("app").factory("FontService", function($http) {
    return {
        fn: function(callback) {
            ...
            $http({
                method: "POST",
                url: window.parent.nbds_frontend.load_font_url,
                params: {
                    action: "nbdesigner_get_fontAction",
                    nonce: window.parent.nbds_frontend.nonce_get
                }
            }).success(function(data, status, headers, config) {
                callback(data);
...
angular.module("app").factory("PatternService", function($q, $http) {
    return {
        fn: function(callback) {
            ...
            $http({
                method: "POST",
                url: window.parent.nbds_frontend.pattern_url,
                params: {
                    action: "nbdesigner_get_patternAction",
                    nonce: window.parent.nbds_frontend.nonce_get
                }
            }).success(function(data, status, headers, config) {
                callback(data);
...
angular.module("app").service("ProductService", function($http, $q) {
    this.getProduct = function(id) {
        var deferred = $q.defer(),
            data = {
                action: "nbdesigner_get_product_infoAction", /* nbdesigner_get_product_info */
                nonce: window.parent.nbds_frontend.nonce,
                id: product_id
            };
        $http({
            url: window.parent.nbds_frontend.url,
            method: "POST",
            data: "action=nbdesigner_get_product_infoAction&nonce=" + window.parent.nbds_frontend.nonce + "&id=" + product_id + "&oid=" + orderid,
            headers: {
...
angular.module("app").factory("QrcodeService", function($http) {
    return {
        fn: function(data, callback) {
            ...
            $http({
                method: "POST",
                url: window.parent.nbds_frontend.get_qrcode_url,
                params: {
                    data: data,
                    action: "nbdesigner_get_qrcodeAction",
                    nonce: window.parent.nbds_frontend.nonce
                }
            }).success(function(data, status, headers, config) {
                callback(data);
...
var NBDESIGNERCART = {
    ...
    upload_design: function(data_img, data_json, fonts, config) {
		...
		var _fonts = JSON.stringify(fonts),
			_config = JSON.stringify(config); /* fix resize */
        var data = {
            "img": data_img,
            "json": data_json
        };
        jQuery.ajax({
            url: window.parent.nbds_frontend.save_customer_design_url,
            method: "POST",
            data: {
                "action": "nbdesigner_save_customer_designAction",
                "image": data,
                "nonce": window.parent.nbds_frontend.nonce,
    ...
    ajax_upload: function(data_img,
        ...
        jQuery.ajax({
            ...
            var _data = JSON.parse(data);
            if (_data.flag == "success") {
                var next_flag = parseInt(self.flag) +
                    1;
                self.flag = next_flag;
                var next_key = "frame_" + self.flag;
                if (self.dataImage[next_key]) {
                    console.log(self.flag);
                    self.ajax_upload(self.dataImage, self.dataJson, self.flag)
                } else {
                ...
                success: function(data) {
                    ...
                    if (!!data.img) {
                        ...
                        $.each(data.img, function(key,
                            value) {
                            html += '<img class="img-responsive img-thumbnail nbdesigner_upload_image shadow hover-shadow" onclick="NBDESIGNERFUNC.addQrImage(\'' + value + '\')" src="' + value + '" />'
                        });
        ...
        if (response.status === "connected") {
            ...
            $.getJSON(photo_json, function() {
                ...
                $.each(photos, function(key, val) {
                    ...
                    html += '<img class="img-responsive img-thumbnail nbdesigner_upload_image shadow hover-shadow" onclick="NBDESIGNERFUNC.addQrImage(\'' + src + '\')" src="' + src + '" />'
                });
                $("#uploaded-facebook").prepend(html);
                if (data.paging.next) {
                    $("#nbdesigner_fb_next").val(data.paging.next);
...
angular.module("app").filter("reverse", function() {
    ...
    return function(input, cat_id) {
        var output = [];
        _.each(input, function(val, key) {
            if (!val.cat) val.cat = [googleCatFont];
