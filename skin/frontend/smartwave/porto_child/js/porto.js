...
/************** Header - Search icon, Links icon click event ***************/
    ...
    $(".qty_inc").unbind('click').click(function(){
		if($(this).parent().parent().children("input.qty").is(':enabled')){
			...
			var _qty;
			if($(this).parent().parent().children("input.qty").val() > 0) {
				_qty = $(this).parent().parent().children("input.qty").val();
			} else {
				 _qty = 1;
			}
			var _changed_price = $("#currency_symbol").val() + (optionsPrice.productPrice*_qty).toFixed(2);
			$(".price").text(_changed_price);
			$("#qty_right").val(_qty);
			//console.log($("input[type='radio']:checked").val());
			$("input[type='radio']:checked").prop("checked", true).trigger("click");
			$("input[type='checkbox']:checked").prop("checked", true).trigger("click");
			$("select option:selected").prop("selected", true).trigger("click");
		}
    });
