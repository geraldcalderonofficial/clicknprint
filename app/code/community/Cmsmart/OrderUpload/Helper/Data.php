	...
	public function integrateOrderUpload()
  	{
		...
		if($attribute == 1){
			...
			$value.='<p>'.Mage::helper('orderupload')->__('File Upload').'</p>';
			$value.='<p>'.Mage::helper('orderupload')->__('Allowable files for upload psd, ai, eps, pdf, jpg Please login to enable file upload.').'</p>';
			$value.='<a class="show_upload"  onclick="';
			$value.="return showFormUpload('upload_files_nbm')";
			$value.='"';
			$value.=' href="#">'.Mage::helper('orderupload')->__('Upload Now').'</a>';
