 ...
 */
class Cmsmart_OrderUpload_Block_Checkout_Cart_Item_Renderer extends Mage_Checkout_Block_Cart_Item_Renderer
{
	/**
					...
					foreach($items as $item)
					{
						...
						/*	$value.='<td>';
								if($data_item['set_comment'] != '')
									$value.='<textarea cols="55" rows="5" id="'.$data_item['set_files']['label'].'" name="'.$data_item['set_files']['label'].'" >'.$data_item['set_comment'].'</textarea>';
								else
									$value.='<textarea cols="55" rows="5" id="'.$data_item['set_files']['label'].'" name="'.$data_item['set_files']['label'].'" placeholder="Extra information" ></textarea>';
							$value.='</td>';
						*/	
							$value.='<td>';
							...
							$value.='<tr>';
							        $value.='<td>';
								if($data_item['set_comment'] != '')
									$value.='<textarea cols="35" rows="5" style="width: 130px; height: 35px; font-size: 11px;" id="'.$data_item['set_files']['label'].'" name="'.$data_item['set_files']['label'].'" >'.$data_item['set_comment'].'</textarea>';
								else
									$value.='<textarea cols="35" rows="5" style="width: 130px; height: 35px; font-size: 11px;" id="'.$data_item['set_files']['label'].'" name="'.$data_item['set_files']['label'].'" placeholder="Extra information" ></textarea>';
							$value.='</td>';
							$value.='</tr>';
